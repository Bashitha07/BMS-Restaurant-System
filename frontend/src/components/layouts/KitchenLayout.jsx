...
const KitchenLayout = ({ children }) => {
  ...
  const handleLogout = () => {
    ...
    if (confirmed) {
      logout();
      toast.success('Kitchen session ended. Logged out successfully.');
      navigate('/', { replace: true });
    }
  };

  const navItems = [
    { path: '/kitchen/dashboard', icon: ChefHat, label: 'Kitchen Dashboard' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              <h1 className="text-2xl font-bold text-accent">BMS Kingdom of Taste - Kitchen</h1>
            </div>
